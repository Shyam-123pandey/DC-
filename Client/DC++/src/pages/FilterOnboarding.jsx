...
import { Container, Box, Typography, FormControl, InputLabel, Select, MenuItem, Button, Paper } from '@mui/material';
...
const subjects = ['Subject 1', 'Subject 2', 'Subject 3']; // Make dynamic if needed
...
const FilterOnboarding = ({ children }) => {
  ...
  const handleChange = (key, value) => {
    setFilters(f => ({ ...f, [key]: value, ...(key === 'program' ? { branch: '', year: '', subject: '' } : {}), ...(key === 'branch' ? { year: '', subject: '' } : {}), ...(key === 'year' ? { subject: '' } : {}) }));
  };
